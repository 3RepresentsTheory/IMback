...
QByteArray readHtmlFile(const QString& filePath) {
    QFile file(filePath);
...
int main(int argc, char *argv[])
{
    ...
    ChatBroadcastServer server(1234);
    ...
    httpServer.route(
            "/chatroom",[&server](){
                return QHttpServerResponse("text/html",
                                           readHtmlFile("../../singleGroupTest/chatclient.html")
                );
            }
    );
//    httpServer.route(
...
//    });
    ...
    const auto port = httpServer.listen(QHostAddress::Any,1235);
    if (!port) {
