...
struct SessionEntry : public Jsonable
{
    ...
    void startSession() { cookie = generatecookie(); }

    void endSession() { cookie = std::nullopt; }

    QJsonObject toJson() const override
    {
        return cookie
                ? QJsonObject{ { "id", id },
                               { "cookie", cookie->toString(QUuid::StringFormat::WithoutBraces) } }
                : QJsonObject{};
    }

    bool operator==(const QString &othercookie) const
    {
        return cookie && *cookie == QUuid::fromString(othercookie);
    }

private:
    QUuid generatecookie() { return QUuid::createUuid(); }
