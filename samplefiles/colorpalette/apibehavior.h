...
public:
    ...
    QFuture<QHttpServerResponse> getPaginatedList(const QHttpServerRequest &request) const
    {
        ...
        return QtConcurrent::run(
                [paginator = std::move(paginator), maybeDelay]()
                {
                        if (maybeDelay)
...
public:
    ...
    bool authorize(const QHttpServerRequest &request) const
    {
        const auto maybecookie = getcookieFromRequest(request);
        if (maybecookie) {
            const auto maybeSession = std::find(sessions.begin(), sessions.end(), *maybecookie);
            return maybeSession != sessions.end() && *maybeSession == *maybecookie;
        }
        return false;
    }
